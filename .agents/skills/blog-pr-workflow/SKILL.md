...
6. `pr-prepare` 또는 `pr-create`이면 PR 제목을 작성한다.
   - PR 본문 파일은 생성하지 않는다.
   - PR 본문 초안은 사용자가 명시적으로 요청한 경우에만 대화 응답으로 작성한다.
   - `.github/PULL_REQUEST_TEMPLATE.md`가 있으면 사용자가 본문 작성을 요청했을 때만 그 구조를 참고한다.
7. `pr-prepare`이면 PR title을 보고하고 멈춘다. `git push`와 `gh pr create`는 실행하지 않는다.
8. `pr-create`이고 현재 브랜치가 base branch와 같으면 `git push` 전에 중단한다. PR용 head branch가 필요하다는 점과 새 브랜치 생성 또는 전환 옵션을 보고한다.
...
11. PR 생성 전 아래 정보를 사용자에게 보여주고 `.codex/rules/remote-operations.md` 형식으로 승인을 받는다.
   ...
   - head branch
   - PR title
   - PR body를 생성하지 않는다는 사실 또는 사용자가 제공한 body 사용 여부
   - 실행할 `gh pr create` 명령
12. 사용자가 명확히 승인한 경우에만 `gh pr create --base master --title "<title>" --body ""`로 PR을 만든다.
    - 사용자가 PR 본문을 직접 제공했거나 본문 작성을 명시적으로 요청한 경우에만 `--body "<body>"`를 사용한다.
