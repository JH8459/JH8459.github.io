  ...
  about: {
    ...
    resume: {
      ...
      introduction: [
        ...
        '운영 환경의 로그, 메트릭, 트레이스를 근거로 병목과 장애를 찾고 데이터베이스, 비동기 이벤트, 배포 구조를 개선해 왔습니다. 시스템을 분리할 때는 서비스 경계와 데이터 소유권, 이벤트 계약, 실패 복구 경로를 함께 설계해 복잡성이 운영 부담으로 이어지지 않도록 합니다.',
        '새로운 기술은 목적이 아니라 문제 해결 수단으로 선택합니다. AI 기능도 모델 연동에 그치지 않고 오탐과 실패가 제품 정책에 미치는 영향을 통제하며, Human-in-the-Loop와 운영 지표를 통해 사람이 검증하고 개선할 수 있는 흐름까지 설계합니다.',
      ],
          ...
          projects: [
            {
              ...
              summary:
                'MAU 약 2만 명의 체험단 플랫폼에서 Next.js API Routes를 NestJS로 무중단 이관하고, 운영 병목을 추적해 데이터·이벤트·배포 구조를 단계적으로 개선하고 있습니다.',
              bullets: [
                '무중단 마이그레이션을 위해 기존 API 계약과 주요 사용자 시나리오를 회귀 테스트로 고정하고 구·신 서버 요청을 단계적으로 전환했습니다. 레거시 데이터는 한시 크론으로 청크 처리하고 병행 조회와 소진 후 제거 절차를 적용해 전환 구간의 정합성을 유지했습니다.',
                '운영자용 캠페인 신청 전체 목록은 약 437만 행의 중간 결과를 만들던 JOINED populate와 정렬을 SELECT_IN 조회와 서브쿼리로 재설계해 p50을 9.63초에서 92ms, p99를 약 30초에서 1.91초로 단축했고, 하루 약 8만 회 호출되는 캠페인 목록 API는 N+1을 일괄 조회로 제거하고 정렬 조건에 맞춘 복합 인덱스를 적용해 요청당 쿼리를 25회에서 4회로, 핵심 쿼리의 스캔 행 수를 164,313건에서 22건으로 줄였습니다.',
                '최대 53분이 걸리던 사용자 안내 메일 발송 크론 배치를 Outbox와 워커 기반 비동기 구조로 전환하고, SMTP 응답을 영구 실패와 재시도 가능 오류로 분류해 불필요한 재시도와 큐 지연을 줄였습니다.',
                '광고주가 체험단의 리뷰 콘텐츠에 연결한 광고 배너의 노출 횟수와 일별 성과를 확인할 수 있도록 기획 단계에서 조회 지표와 집계 흐름을 설계했습니다. Kafka 멱등 컨슈머로 개별 노출 이력과 일별 집계 데이터를 생성하고, 장애 격리, 누락 지표, SigNoz 트레이스를 구성해 사용자 요청부터 이벤트 처리까지 추적할 수 있도록 했습니다.',
                'Kubernetes·Argo CD 롤링 배포 중 진행 중인 요청과 작업이 중단되는 위험을 확인해 NestJS graceful shutdown과 Helm 종료 대기를 적용했습니다.',
              ],
            },
            {
            ...
            },
            {
              ...
              summary:
                '회원, 프로필, 커뮤니티, 포인트, 캠페인을 독립 서비스로 분리하며 경계와 데이터 소유권을 정의하고, Kafka 기반 상태 동기화, 이벤트 계약, 데이터 정합성, 실패 복구, CI까지 MSA의 공통 기반을 설계했습니다.',
              bullets: [
                '도메인별 서비스 경계와 데이터 소유권을 정의하고 서비스 간 동기 호출을 Kafka 이벤트 기반 상태 동기화로 전환했습니다. 각 서비스가 필요한 데이터를 직접 소유하도록 구성해 결합도와 장애 전파 범위를 낮췄으며, 상태 재구성이 필요한 기능에는 Event Sourcing을 적용했습니다.',
                '서비스별로 달랐던 이벤트 이름·페이로드·버전 규칙을 공통 계약으로 정의해 Git 서브모듈로 관리하고, CI 스키마 검증으로 규격 불일치를 병합 전에 차단했습니다.',
                'Kafka 비동기 처리의 유실·중복 가능성에 대응해 Outbox/Inbox·멱등 처리·실패 이벤트 격리·재처리 API를 구성하고 운영 복구 경로를 마련했습니다.',
                'lint·단위 테스트·빌드·도메인별 격리 E2E를 공통 CI로 통합해 평균 실행 시간을 67.9% 단축했습니다.',
              ],
            },
            {
