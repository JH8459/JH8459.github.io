          ...
          projects: [
            {
              ...
              bullets: [
                ...
                '캠페인 조회 쿼리를 SELECT_IN·서브쿼리·복합 인덱스로 재설계해 운영자용 신청 목록의 p50을 9.63초에서 92ms로, p99를 약 30초에서 1.91초로 단축했습니다. 운영 환경에서 SigNoz로 목록 API의 쿼리 병목을 확인하고 인덱스를 개선해, 핵심 목록 쿼리의 스캔 행 수를 164,313건에서 22건으로 줄였습니다. 하루 약 8만 회 호출되는 목록 API의 N+1도 제거해 요청당 쿼리 수를 25회에서 4회로 낮췄습니다.',
                '사용자 안내 메일 크론 배치가 최대 약 53분간 실행되는 상황을 확인해, 메일 발송 요청을 Outbox에 저장하고 워커가 처리하는 비동기 구조로 전환했습니다. SMTP 응답을 영구 실패와 재시도 가능 오류로 분류해 불필요한 재시도와 큐 적체를 막고, 발송 직전에 수신처를 조회해 개인정보 삭제 조건을 반영했습니다.',
