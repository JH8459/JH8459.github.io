...
const metaConfig: MetaConfig = {
  ...
  about: {
    ...
    resume: {
      ...
      email: 'kk_ong2233@naver.com',
      introduction: [
        'Node.js와 TypeScript를 중심으로 B2B 채용·검사 솔루션과 B2C 광고·주문 플랫폼을 모두 경험한 5년 차 백엔드 엔지니어입니다. 고객사마다 다른 요구를 공통 구조로 풀어내는 일과, 사용자 행동이 결제·정산으로 이어지는 흐름을 안정적으로 연결하는 일에 익숙합니다.',
        '기능을 구현하기 전에 사용자와 운영자가 어떤 순간에 불편을 겪는지 먼저 살피고, 실패했을 때 누가 어떻게 복구할지까지 제품의 일부로 설계합니다. 복잡한 문제일수록 로그·메트릭·트레이스와 현장의 사용 흐름을 함께 보며 원인을 좁히고, 팀이 오래 운영할 수 있는 단순한 기준을 남기려 합니다.',
        '새로운 기술은 문제에 맞을 때 선택하고 선택한 이유를 동료가 검증할 수 있도록 기록합니다. AI 역시 사람을 대체하는 기능보다 사람이 더 정확하게 판단하도록 돕는 도구로 바라보며, 제품 기능과 개발 과정 모두에 검토와 관측이 가능한 흐름을 만드는 개발자입니다.',
      ],
          ...
          projects: [
            {
              ...
              summary:
                'MAU 약 2만 명의 체험단 플랫폼에 기획 단계부터 참여해 백엔드 구조와 운영 흐름의 개선을 주도했습니다. Next.js API Routes 기반 v1을 NestJS 기반 v2로 무중단 이관한 뒤 광고 노출·알림·결제처럼 정산과 운영에 연결되는 기능을 지속적으로 고도화하고 있습니다.',
              bullets: [
                '기존 API 계약과 주요 사용자 시나리오를 TDD 기반 회귀 테스트로 고정한 뒤 구·신 서버 요청을 단계적으로 전환했습니다. 레거시 데이터는 처리 주기와 전환 일정을 계산해 청크 단위로 이관하는 크론을 설계하고, 병행 조회로 하위 호환성을 유지한 뒤 데이터가 소진된 시점에 구 경로를 제거해 서비스 중단 없이 v1에서 v2로 마이그레이션했습니다.',
                '광고 배너 조회 수가 봇·중복·조작 요청으로 부풀지 않도록 서명 검증과 봇·작성 화면 필터를 선행하고, Redis 원자 연산으로 동일 열람자의 중복을 30분간 억제했습니다. 검증된 노출은 CQRS 이벤트 버스에서 Kafka로 전달해 Consumer Inbox로 멱등 처리하고, 적재 실패는 보상 처리하며 최근 3일을 재집계해 지연 도착과 부분 실패에도 일별 광고 성과의 정합성을 유지했습니다.',
                'Next.js API Routes 기반 v1의 운영자용 캠페인 신청 목록은 JOINED populate와 정렬로 약 437만 행 테이블을 전수 스캔해 30초 타임아웃과 디스크 부족이 발생했습니다. NestJS 기반 v2에서 SELECT_IN 조회와 서브쿼리로 재설계해 p50을 9.63초에서 92ms로, p99를 약 30초에서 1.91초로 단축했습니다. 하루 약 8만 회 호출되는 캠페인 목록 API도 N+1 제거와 복합 인덱스로 요청당 쿼리를 25회에서 4회로, 스캔 행 수를 164,313건에서 22건으로 줄였습니다.',
            ...
            },
            {
              ...
              bullets: [
                '상품 주문 후기를 댓글로 작성할 수 있는 기능을 추가하고, 작성된 댓글을 검수 대기 상태로 저장한 뒤 비동기 워커가 Gemini 판정을 수행하도록 구현했습니다. 판정 결과에 따라 승인, 보류, 지연 상태로 전이시키고 승인된 댓글만 사용자에게 배정했으며, AI가 보류한 댓글은 관리자가 최종 차단 여부를 확인하는 Human-in-the-Loop(HITL) 검수 흐름으로 연결했습니다.',
                '댓글별 승인과 보류 판정, Gemini 호출 실패 원인, 지연 상태 전환 건수, 검수 대기 시간을 OTel 지표로 계측하고 SigNoz에서 운영 상태를 추적할 수 있도록 구현했습니다. 배포 후 실트래픽에서 집계된 차단율 0.73%를 초기 운영 기준선으로 설정해 차단율이 급증하면 프롬프트의 판정 쏠림을, 실패와 지연 지표가 증가하면 모델 호출이나 워커의 장애를 구분해 확인할 수 있도록 했습니다.',
            ...
            },
            {
              ...
              summary:
                'PostgreSQL을 사용하는 회원·프로필·커뮤니티·포인트·캠페인 서비스를 분리하며 데이터 소유권과 이벤트 기반 운영 기준을 설계했습니다.',
              bullets: [
                '서비스가 서로의 데이터를 직접 조회해 변경과 장애가 연쇄 전파되는 문제를 줄이기 위해 업무 경계와 데이터 소유권을 나누고 Kafka 이벤트로 필요한 상태만 동기화했습니다. 상태 재구성이 필요한 기능은 이벤트 기록에서 복원할 수 있게 해 서비스가 독립적으로 변경되고 복구되는 기반을 마련했습니다.',
                '서비스마다 다른 이벤트 형식 때문에 배포 후 호환성이 깨질 위험이 있어 이름·데이터·버전 규칙을 공통 계약으로 만들고 CI에서 변경을 검증했습니다. Outbox·Inbox와 중복 방지, 실패 격리·재처리 API까지 함께 구성해 메시지 유실과 중복이 생겨도 운영자가 복구할 수 있게 했습니다.',
                '서비스별 검증 절차가 달라 배포 준비 시간이 길어지자 lint·단위 테스트·빌드·도메인별 격리 E2E를 공통 CI로 통합했고, 병렬 실행을 적용해 평균 실행 시간을 67.9% 단축했습니다.',
              ],
            },
            {
          ...
          projects: [
            {
              ...
              bullets: [
                '고객사마다 검사 콘텐츠·결과·연동 규격이 달라 도입 때마다 개별 개발이 반복되자 문제·응답·채점·결과 모델과 B2B API를 공통 규격으로 정리했습니다. 고객사별 차이는 같은 구조 안의 설정과 규칙으로 수용해 신규 도입의 반복 작업과 변경 범위를 줄였습니다.',
                '데이터가 늘수록 대량 텍스트 검색이 느려져 검색 역할을 Elasticsearch로 분리했고, DB가 맡던 부하를 낮춰 응답 속도를 약 4배 개선했습니다.',
                '배포 때마다 서비스가 중단되고 장애가 나면 개별 로그만으로 요청 흐름을 파악하기 어려웠습니다. Docker Compose 기반 블루·그린 배포를 도입해 중단 없이 새 버전을 배포하고 필요하면 기존 버전으로 전환할 수 있게 했으며, 로그, 메트릭, 트레이스를 연결해 장애 요청의 흐름과 원인을 추적할 수 있는 운영 환경을 구축했습니다.',
              ],
            },
            {
              ...
              bullets: [
                ...
                '대용량 녹화 파일이 서버를 거치며 부하와 전송 비용을 높여 S3 직접 업로드로 경로를 바꾸고 Lambda·EventBridge로 보관 작업을 자동화했습니다. 응시 흐름과 녹화 처리를 분리해 시험 중 서버 부담과 운영자의 수작업을 함께 줄였습니다.',
              ],
            },
            {
              title: 'Electron 기반 부정행위 방지 검사 애플리케이션 개발 및 운영 표준화',
              bullets: [
                '브라우저만으로 응시자 PC를 통제하기 어려워 Electron 앱에 듀얼 모니터 감지와 특정 프로그램 차단을 구현했습니다. 자동 업데이트와 오류 로그 형식도 표준화해 여러 고객 환경에서 같은 기준으로 배포하고 장애를 재현할 수 있게 했습니다.',
              ],
            },
          ],
        },
      ...
      skills: [
        {
          category: 'Backend',
          items: ['Node.js', 'TypeScript', 'NestJS', 'Express', 'Next.js', 'Fastify', 'REST API'],
        },
        {
