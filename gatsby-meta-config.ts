...
const metaConfig: MetaConfig = {
  ...
  about: {
    ...
    resume: {
      ...
      email: 'kk_ong2233@naver.com',
      introduction: [
        'Node.js와 TypeScript를 기반으로 B2B 채용·검사 솔루션과 B2C 광고·주문 플랫폼을 개발·운영해 온 5년 차 백엔드 엔지니어입니다.',
        'WebSocket·WebRTC 기반 실시간 화상 감독 기능을 개발하고, 고객사마다 다른 검사 콘텐츠와 결과·연동 규격을 공통 데이터 모델과 B2B API로 구조화했습니다. AWS 기반 녹화 처리와 관측 환경을 구성해 집중 부하와 장애 징후를 운영 가능한 흐름으로 바꿨습니다.',
        '회귀 테스트와 최소 재현으로 변경을 검증하고, 로그·메트릭·트레이스로 배포와 장애 대응까지 책임집니다. AI 기능도 사람의 최종 판단과 운영 지표가 남는 구조로 설계합니다.',
      ],
          ...
          projects: [
            {
              ...
              bullets: [
                ...
                '광고 노출은 서명·봇 필터 후 Redis 원자 연산으로 30분간 중복을 억제하고 Kafka·Consumer Inbox로 멱등 적재했습니다. 실패 시 보상 처리하고 최근 3일을 재집계해 지연 도착과 부분 실패에도 일별 성과의 정합성을 유지했습니다.',
        ...
        },
        {
          ...
          projects: [
            ...
            },
            {
              ...
              bullets: [
                'LG Way Fit을 포함한 대기업 고객사의 채용·검사 서비스 개발에 참여했습니다. 고객사마다 달랐던 검사 콘텐츠·결과·연동 규격을 문제·응답·채점·결과의 공통 데이터 모델과 B2B API로 구조화하고, 차이는 설정과 규칙으로 수용했습니다.',
                '데이터가 늘수록 대량 텍스트 검색이 느려져 검색 역할을 Elasticsearch로 분리했고, DB가 맡던 부하를 낮춰 응답 속도를 약 4배 개선했습니다.',
                '배포 때마다 서비스가 중단되고 장애가 나면 개별 로그만으로 요청 흐름을 파악하기 어려웠습니다. Docker Compose 기반 블루·그린 배포와 로그·메트릭·트레이스 관측 환경을 구성해 중단 없이 배포하고 장애 요청의 원인을 추적할 수 있게 했습니다.',
              ],
            },
            {
