      ...
      experiences: [
        {
          ...
          projects: [
            ...
            },
            {
              ...
              bullets: [
                '주문 유형과 외부 공급자마다 상태 체계가 달라 실패, 환불, 재처리 기준이 일관되지 않았습니다. 공급자별 상태를 내부 주문 생명주기에 매핑하고 상태 전이 조건을 도메인 규칙으로 정의해, 주문 상태와 후속 처리 기준을 하나의 모델로 통합했습니다.',
                '주문 옵션 조회의 p99가 25.3초까지 증가하는 병목을 관측했습니다. 상품 옵션, 쿠폰, 결제, 포인트 조회 과정의 N+1과 불필요한 JOIN을 원인으로 확인하고 조회 목적별 쿼리로 분리해, p99를 0.58초로 97.7% 단축했습니다.',
                '주문 상태 변경과 Kafka 이벤트 발행이 분리되어 한쪽만 성공하면 데이터와 이벤트가 불일치할 수 있었습니다. Kafka Outbox로 상태 변경과 이벤트 기록을 동일한 DB 트랜잭션에 묶고 비동기 발행과 재시도를 구성해, 이벤트 유실 가능성을 낮추고 주문 상태와 이벤트의 정합성을 강화했습니다.',
                '배포 후 주문 API와 외부 공급자 연동의 성능 변화를 일관되게 검증하기 위해 SigNoz MCP를 AI Agent에 연결했습니다. AI Agent가 호출량, 평균 응답 시간, p99, 에러율을 배포 전후로 비교하고 이상 징후를 알림으로 요약하도록 구성해 운영 지표 확인 흐름을 표준화했습니다.',
                'AI Agent가 생성한 코드의 품질 편차와 기존 API 회귀 가능성을 줄이기 위해 저장소 규칙, 도메인 맥락, 테스트·리뷰 기준을 작업 맥락으로 구조화했습니다. 계획 수립, 구현, 테스트, 정적 검사, 리뷰를 5단계 검증 흐름으로 연결해 코드 작성부터 PR까지 동일한 품질 기준을 적용했습니다.',
              ],
            },
          ],
        },
