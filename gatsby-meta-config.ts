      ...
      experiences: [
        {
          ...
          projects: [
            ...
            },
            {
              ...
              bullets: [
                '서비스별 발행·소비 규격을 공통 계약으로 정의하고 Git 서브모듈에서 관리했습니다. 계약 정합성 검사를 CI에 연결해 발행자와 소비자의 규격 불일치를 병합 전에 검증했습니다.',
                '데이터 변경과 이벤트 발행 사이의 유실·중복을 다루기 위해 Outbox·Inbox, 멱등 처리, 재시도, 실패 메시지 격리를 적용했습니다. 발행 정보를 보존하고 재처리 API를 구성해 실패한 이벤트를 운영자가 재처리할 수 있게 했습니다.',
                '서비스 간 동기 호출을 줄이기 위해 변경 이벤트로 필요한 데이터를 각 서비스에 동기화했습니다. 변경 이력과 상태 재구성이 필요한 기능에는 Event Sourcing을 선별 적용해, 이벤트 재생으로 상태를 복원할 수 있게 했습니다.',
                'E2E 테스트의 데이터와 외부 환경을 도메인별로 격리해 CI 병렬 실행 기반을 마련하고 평균 실행 시간을 67.9% 단축했습니다.',
              ],
            },
            {
